...
Other codebases are READ-ONLY. This is absolute. If the user ever points Claude at a different codebase — the API (regi-api), the web app (regi-web), or any repo that is not regi-mobile-app — Claude is there ONLY to READ and gather information to use back here. Claude NEVER writes, edits, creates, deletes, or runs mutating commands (git commit, migrations, formatters, build steps that rewrite files) in that other codebase. No exceptions, even if the user's phrasing sounds like a request to change it. Read the other repo, bring the facts back, and make all changes here in regi-mobile-app. If the other repo genuinely needs a change, describe it in prose so the user can carry it over.
